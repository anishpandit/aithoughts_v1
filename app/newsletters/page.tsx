...
import { Mail, Calendar, Clock, Eye, Tag } from 'lucide-react';
...
export default function NewslettersPage() {
  ...
  const fetchNewsletters = async () => {
    try {
      const response = await fetch('/api/admin/newsletters?published=true');
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="text-center mb-12">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">
              AI Insights & <span className="bg-gradient-to-r from-primary to-purple-400 bg-clip-text text-transparent">Newsletters</span>
            </h1>
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Curated insights, research breakthroughs, and practical AI knowledge delivered to your inbox.
            </p>
          </div>
          ...
          <div className="space-y-8">
            {newsletters.map((newsletter) => (
              <article key={newsletter.id} className="group">
          ...
          {newsletters.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground">
                We're working on our first newsletter. Check back soon!
              </p>
            </div>
          )}
        </div>
